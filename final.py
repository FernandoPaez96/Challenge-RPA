...
class Automation:
    ...
    def write_excel(self):
        ...
        self.file.create_workbook("self.agencies.xlsx")
        self.file.create_worksheet("self.agencies")   
        self.file.set_active_worksheet("self.agencies")
        for n in self.text:
    ...
    def table(self):
        ...
        time.sleep(10)
        self.browser_lib.set_browser_implicit_wait(10)
        self.file.open_workbook("self.agencies.xlsx")
        try:
    ...
    def donwload_pdf_if_exists(self):
        ...
        links = []
        for n in range(1,int(entries)+1):
            elems = self.browser_lib.find_elements('xpath://*[@id="investments-table-object"]/tbody/tr[{}]/td[1]'.format(n))                                
            for n in elems:
                links.append(n.text)

        current_url = (self.browser_lib.get_location()) + "/"
        ...
        for n in links:
            ...
            time.sleep(5)
            try:
                self.browser_lib.click_element_when_visible('xpath://*[@id="business-case-pdf"]/a')
                time.sleep(10)
            except(AssertionError):
                print(f"{n} dont have link")
